...
set +x
  ...
  echo
  set -x
  fabric-ca-client register --caname ca-org1 --id.name org1admin --id.secret org1adminpw --id.attrs 'participantType=admin:ecert,adminId=1' --id.type admin --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
  ...
  echo
  set -x
	fabric-ca-client enroll -u https://org1admin:org1adminpw@localhost:7054 --caname ca-org1 --enrollment.attrs "participantType,adminId" -M ${PWD}/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
  set +x
...
set -x
    echo "================ ORG2IDENTITIES =========================="
fabric-ca-client register -d --id.name universityTecnicoLisboa --id.affiliation org2 --id.attrs '"hf.Registrar.Roles=peer,client"' --id.attrs hf.Revoker=true,participantType=university,entityId=2,organizationName=universityTecnicoLisboa  --id.secret pw --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
fabric-ca-client enroll -u https://universityTecnicoLisboa:pw@localhost:8054 --enrollment.attrs "participantType,entityId,organizationName" --caname ca-org2 -M ${PWD}/organizations/peerOrganizations/org2.example.com/users/universityTecnicoLisboa@org2.example.com/msp --tls.certfiles ${PWD}/organizations/fabric-ca/org2/tls-cert.pem
