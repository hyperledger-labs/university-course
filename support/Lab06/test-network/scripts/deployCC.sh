...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o localhost:7050 -C $CHANNEL_NAME -n b4s $PEER_CONN_PARMS  -c '{"function":"org.b4s.quc:showIdentity","Args":[]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n b4s $PEER_CONN_PARMS -c '{"function":"org.b4s.quc:showIdentity","Args":[]}' >&log.txt
    res=$?
    set +x
	fi
  cat log.txt
