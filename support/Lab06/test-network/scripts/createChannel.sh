...
createChannelTx() {
  ...
  #chmod 777 -R ../organizations
	...
	#todo adicionado
	#chmod 777 ../channel-artifacts/${CHANNEL_NAME}.tx
	res=$?
	set +x
	if [ $res -ne 0 ]; then
...
createChannel() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
        ...
        set -x
				#sudo chmod 766 channel-artifacts/${CHANNEL_NAME}.tx
				peer channel create -o localhost:7050 -t 15s -c $CHANNEL_NAME -f ./channel-artifacts/${CHANNEL_NAME}.tx --outputBlock ./channel-artifacts/${CHANNEL_NAME}.block >&log.txt
				res=$?
        set +x
		else
				set -x
				...
				#sudo chmod 766 channel-artifacts/${CHANNEL_NAME}.tx
				#chmod 766 channel-artifacts/${CHANNEL_NAME}.block
				peer channel create -o localhost:7050 -c $CHANNEL_NAME --ordererTLSHostnameOverride orderer.example.com -t 15s -f ./channel-artifacts/${CHANNEL_NAME}.tx --outputBlock ./channel-artifacts/${CHANNEL_NAME}.block --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA >&log.txt
				res=$?
				set +x
		fi
		let rc=$res
